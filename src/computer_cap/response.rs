//! Computer bridge response normalization.
...
fn bridge_ok(
    ...
    Err(format!("computer.{}: {}", method, message(map)))
}
...
fn tool_success(
    ...
    if !success {
        return Err(format!("computer.{}: {}", method, message(map)));
    }
    match map.get("output") {
        Some(Value::Str(s)) => Ok(json::parse_str(s).unwrap_or_else(|_| Value::Str(s.clone()))),
        Some(v) => Ok(v.clone()),
