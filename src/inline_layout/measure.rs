...
pub fn measure_inline_box(b: &InlineBox) -> (f32, f32) {
    match &b.kind {
        InlineBoxKind::Text(run) => (
            measure_text(&run.text),
            run.font_size * LINE_HEIGHT_FACTOR,
        ),
        InlineBoxKind::InlineBlock { width, height }
        | InlineBoxKind::Image { width, height } => (*width, *height),
...
pub fn ascent(b: &InlineBox) -> f32 {
    match &b.kind {
        ...
        InlineBoxKind::InlineBlock { height, .. }
        | InlineBoxKind::Image { height, .. } => *height,
...
pub fn descent(b: &InlineBox) -> f32 {
    match &b.kind {
        ...
        InlineBoxKind::InlineBlock { .. } | InlineBoxKind::Image { .. } | InlineBoxKind::LineBreak => 0.0,
    }
}
