...
fn detects_regions() {
    let els = vec![ElementSummary {
        selector: "nav".into(), tag: "nav".into(), ..Default::default()
    }];
...
fn classifies_login_form() {
    let inputs = vec![
        InputSummary { input_type: "email".into(), name: Some("email".into()), ..Default::default() },
        InputSummary { input_type: "password".into(), name: Some("password".into()), ..Default::default() },
    ];
...
fn classifies_download_link() {
    assert_eq!(links::classify_link("/report.pdf", "Download report", "main"), LinkKind::Download);
}

#[test]
fn detects_actionable_button() {
    let els = vec![ElementSummary {
        selector: "#save".into(), tag: "button".into(), text: "Save".into(), ..Default::default()
    }];
