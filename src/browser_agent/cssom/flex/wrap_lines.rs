...
pub fn lines(items: &mut [FlexItem], row: bool, main: i64, flex_wrap: FlexWrap)
    -> Vec<(usize, usize, i64)>
{
    if matches!(flex_wrap, FlexWrap::NoWrap) || items.is_empty() {
        size::size_line(items, row, main);
        let c = items.iter().map(|i| resolve::cross_size(i, row)).max().unwrap_or(0);
    ...
    cross = items[s..].iter().map(|i| resolve::cross_size(i, row)).max().unwrap_or(0);
    out.push((s, items.len(), cross));
    if matches!(flex_wrap, FlexWrap::WrapReverse) { out.reverse(); }
    out
}
