...
pub fn size_line(items: &mut [FlexItem], row: bool, container_main: i64) {
    ...
    for it in items.iter_mut() {
        let b = it.flex_basis.unwrap_or_else(|| resolve::main_size(it, row))
            .max(it.min_size).min(it.max_size);
    ...
    if free > 0 {
        ...
        if grow > 0.0 { for it in items {
            let add = ((free as f64) * it.flex_grow / grow).round() as i64;
            resolve::set_main(it, row, (resolve::main_size(it, row) + add).min(it.max_size));
        }}
    } else if free < 0 {
        let scaled: f64 = items.iter().zip(base.iter())
            .map(|(i, b)| i.flex_shrink * (*b as f64)).sum();
        if scaled > 0.0 { for (it, b) in items.iter_mut().zip(base) {
