...
pub fn num(s: &HashMap<String, String>, key: &str, default: f64) -> f64 {
    val(s, key).and_then(|v| v.parse::<f64>().ok()).unwrap_or(default)
}

pub fn len(s: &HashMap<String, String>, key: &str) -> Option<i64> {
    val(s, key).and_then(|v| parse_len(&v))
}

pub fn parse_len(v: &str) -> Option<i64> {
    let v = v.trim();
    if v == "auto" { return None; }
    v.trim_end_matches("px").parse::<f64>().ok().map(|n| n.round() as i64)
}

fn val(s: &HashMap<String, String>, key: &str) -> Option<String> {
