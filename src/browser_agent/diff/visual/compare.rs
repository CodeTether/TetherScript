...
pub fn compare_rasters(a: &RasterImage, b: &RasterImage) -> DiffResult {
    ...
    DiffResult { stats, mask, size_mismatch }
}

fn stats_for(mask: &DiffMask) -> DiffStats {
    let changed = mask.changed.iter().filter(|&&c| c).count();
    let total = mask.width * mask.height;
    let pct = if total == 0 { 0.0 } else { changed as f64 * 100.0 / total as f64 };
    DiffStats { total_pixels: total, changed_pixels: changed, max_color_distance: 1020, diff_percentage: pct }
}
