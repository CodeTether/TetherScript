...
pub struct SelectorHealCache { entries: HashMap<String, ElementProps> }
...
impl SelectorHealCache {
    ...
    pub fn get(&self, selector: &str) -> Option<&ElementProps> { self.entries.get(selector) }
    pub fn remove(&mut self, selector: &str) -> Option<ElementProps> { self.entries.remove(selector) }
