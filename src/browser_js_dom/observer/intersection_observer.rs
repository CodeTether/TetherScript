...
impl IntersectionObserver {
    pub fn new(thresholds: Vec<f64>, callback: IntersectionCallback) -> Self {
        let mut t: Vec<f64> = if thresholds.is_empty() { vec![0.0] }
            else { thresholds.into_iter().map(|v| v.clamp(0.0, 1.0)).collect() };
        t.sort_by(|a, b| a.partial_cmp(b).unwrap()); t.dedup();
        Self { thresholds: t, targets: HashMap::new(), callback }
    }
    pub fn observe(&mut self, id: u64) { self.targets.entry(id).or_insert(-1.0); }
    ...
    pub fn check<F: Fn(u64) -> Option<Rect>>(&mut self, root: Rect, bounds: F) {
        ...
        for (&id, last) in self.targets.iter_mut() {
            if let Some(rect) = bounds(id) {
                ...
                if crossed(*last, entry.intersection_ratio, &self.thresholds) {
...
fn crossed(old: f64, new: f64, thresholds: &[f64]) -> bool {
    old < 0.0 || thresholds.iter().any(|&t|
