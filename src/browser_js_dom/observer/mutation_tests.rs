...
fn mutation_delivery_filters_by_attribute_name() {
    let seen = Rc::new(RefCell::new(0)); let out = seen.clone();
    let mut reg = MutationObserverRegistry::new();
    reg.observe(1, MutationObserverConfig {
        attributes: true, attribute_filter: vec!["class".into()], ..Default::default()
    }, Box::new(move |r| { *out.borrow_mut() += r.len(); }));
...
fn mutation_subtree_matches_ancestor() {
    let seen = Rc::new(RefCell::new(false)); let out = seen.clone();
    let mut reg = MutationObserverRegistry::new();
    reg.observe(1, MutationObserverConfig {
        child_list: true, subtree: true, ..Default::default()
