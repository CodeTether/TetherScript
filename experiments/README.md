...
Answers the question: **do models emit valid tetherscript source that accomplishes
...
distilled specialist models for tetherscript generation.
...
| SEMANTIC   | Parses but runtime fails (wrong method, unhandled `?`, capability violation, …). | Capability surface is rough, or model needs training data on tetherscript semantics. |
...
tetherscript --prompt fetch-and-save > prompt.txt

# 2. Feed it to whatever model you're testing. Capture its reply
#    (which should be tetherscript source) to a file:
cat prompt.txt | your-model-cli > model_output.tether

# 3. Classify:
tetherscript --experiment fetch-and-save model_output.tether
...
for f in outputs/*.tether; do
    tetherscript --experiment fetch-and-save "$f" 2>> trajectories.jsonl 1>/dev/null
done
