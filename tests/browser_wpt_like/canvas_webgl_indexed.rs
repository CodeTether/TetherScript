...
pub(super) fn run() {
    ...
    let script = "let gl=document.getElementById('gl').getContext('webgl');\
        ...
        gl.drawElements(gl.TRIANGLES,6,gl.UNSIGNED_SHORT,0);let pixel=new Uint8Array(4);\
        gl.readPixels(1,1,1,1,gl.RGBA,gl.UNSIGNED_BYTE,pixel);pixel.join(',');";
    let mut page = BrowserPage::from_html("mem://webgl-indexed", html);
    ...
    assert_eq!(webgl.commands.last().unwrap().operation, "drawElements");
    assert_eq!(webgl.commands.last().unwrap().args, ["4", "6", "5123", "0"]);
}
