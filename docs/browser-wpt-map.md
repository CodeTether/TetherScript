...
| Canvas/WebGL | Native RGBA canvas pixels, ImageData reads/writes, WebGL shader/program lifecycle, typed-array vertex/element buffers, constant/uniform fragment colors, `drawArrays`/`drawElements` triangles, clears/readback, viewport/scissor state, channel write masks, and sticky GL errors | `html/canvas`, `webgl` | `src/browser_js_canvas/webgl_tests_elements.rs`, `src/browser_js_canvas/webgl_tests_elements_webgl2.rs`, `tests/browser_wpt_like/canvas_webgl.rs` | WPT-like software-rendered subset | Textures, depth/stencil, blending, general GLSL ES, and GPU acceleration remain unsupported |
