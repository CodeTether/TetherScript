...
| Selectors/CSS cascade | Type/id/class/attribute selectors, all combinators, structural pseudos, specificity, inline precedence, and native-page width/color-scheme/reduced-motion/forced-colors `@media` rules | `css/selectors`, `css/css-cascade`, `css/mediaqueries` | `src/browser_agent/cssom/media_tests.rs`, `tests/browser_css_media_parity.rs` | WPT-like deterministic subset | Standalone `browser_eval_js` computed styles do not project `@media`; no origins, layers, `!important`, complete inheritance/shorthand expansion, container queries, or unknown-value handling |
...
| Canvas/WebGL | Native RGBA canvas pixels, ImageData reads/writes, WebGL shader/program lifecycle, typed-array vertex buffers, constant/uniform fragment colors, `drawArrays` triangles, clears/readback, viewport/scissor state, channel write masks, and sticky GL errors | `html/canvas`, `webgl` | `src/browser_js_canvas/webgl_tests_draw.rs`, `src/browser_js_canvas/webgl_tests_uniform.rs`, `tests/browser_wpt_like/canvas_webgl.rs` | WPT-like software-rendered subset | Textures, indexed drawing, depth/stencil, blending, general GLSL ES, and GPU acceleration remain unsupported |
