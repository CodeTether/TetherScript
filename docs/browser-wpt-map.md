...
| Events/Input | `addEventListener`, `removeEventListener`, `onclick`, `dispatchEvent`, `click`, event `type`/`target`, listener `this`, composition event data, touchstart/touchmove/touchend metadata, PointerEvent pointerType differentiation | `dom/events`, `uievents`, `pointerevents`, `touch-events` smoke cases | `src/browser_js.rs::event_listeners_property_handlers_this_and_event_target_work`, `tests/browser_wpt_like/keyboard_pointer.rs` | WPT-like deterministic subset | Composed paths, trusted events, and complete hardware input negotiation remain partial |
