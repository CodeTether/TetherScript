...
| Scrolling | Viewport wheel offsets; element `scroll`, `scrollTo`, `scrollBy`, `scrollLeft`, `scrollTop`, and `scrollIntoView`; clamped overflow geometry; scroll events; scrolled hit testing | `css/cssom-view`, `uievents/wheel` | `src/browser_js_dom/scroll_metrics_tests.rs`, `tests/browser_wpt_like/wheel_scroll.rs` | WPT-like deterministic subset | Smooth scrolling animation and scrollbar hit testing remain unsupported |
